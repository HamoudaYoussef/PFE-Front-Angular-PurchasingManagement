...
@Component({
  selector: 'app-grid-produit-offert',
  templateUrl: './grid-produit-offert.component.html',
  styleUrls: ['./grid-produit-offert.component.scss']
})
export class GridProduitOffertComponent implements OnInit {
  ...
  getRequestCaseLazy() {
    ...
    this.dataSourceElement = new CustomStore({
          ...
          load: function (loadOptions: any) {
            ...
            return this.httpClient.get(this.env.urlProject + 'produit-offerts/produit-offerts-pagination?' + params, {headers: new HttpHeaders().set("Authorization", this.tokenStorage.getToken()).append("application", require('package.json').name)})
                .toPromise()
                .then((data: any) => {
